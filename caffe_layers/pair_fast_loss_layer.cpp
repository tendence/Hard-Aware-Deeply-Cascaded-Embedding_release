...
namespace caffe {
	bool ppsort(const pair<float, pair<int, int> >& a, const pair<float, pair<int, int> >& b)
	{
	...
	void PairFastLossLayer<Dtype>::Forward_cpu(
		...
		for (map<int, vector<int> >::iterator itr1=label_data_map.begin();itr1!=label_data_map.end();itr1++)
		{
			//int cur_size = label_data_map[ent1.first].size();
			int cur_size = label_data_map[itr1->first].size();
			if (cur_size < 2)
			{
				continue;
			}
			...
			for (int j = 0; j < cur_size - 1; j++)
			{
				for (int k = j + 1; k < cur_size; k++)
				{
					...
					int anc = label_data_map[itr1->first][j];
					int pos = label_data_map[itr1->first][k];
					...
					for (map<int, vector<int> >::iterator itr2=label_data_map.begin();itr2!=label_data_map.end();itr2++)
					{
						//if (ent1.first == ent2.first) continue;
						if (itr1->first == itr2->first) continue;
						//int m_size = label_data_map[ent2.first].size();
						int m_size = label_data_map[itr2->first].size();
						for (int n = 0; n < m_size; n++)
						{
							triplet_count += Dtype(2);
							//int neg = label_data_map[ent2.first][n];
							int neg = label_data_map[itr2->first][n];
		...
		//for (std::pair<int, vector<int> > const &ent1 : label_data_map)
		for (map<int, vector<int> >::iterator itr1=label_data_map.begin();itr1!=label_data_map.end();itr1++)
		{
			//int cur_size = label_data_map[ent1.first].size();
			int cur_size = label_data_map[itr1->first].size();
			...
			for (int i = 0; i < cur_size - 1; i++)
			{
				for (int j = i + 1; j < cur_size; j++)
				{
					...
					int pos_1 = label_data_map[itr1->first][i];
					int pos_2 = label_data_map[itr1->first][j];
			...
			if (use_pos != 1) {
				...
				for (int i = 0; i < cur_size; i++)
				{
					...
					//for (std::pair<int, vector<int> > const &ent2 : label_data_map)
					for (map<int, vector<int> >::iterator itr2=label_data_map.begin();itr2!=label_data_map.end();itr2++)
					{
		...
		if (hard_ratio < 1) {
			sort(hard_loss_pos.begin(), hard_loss_pos.end(), ppsort);
			if (use_pos != 1) {
				sort(hard_loss_neg.begin(), hard_loss_neg.end(), ppsort);
			}
		}
